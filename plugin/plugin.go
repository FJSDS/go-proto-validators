...
import (
	...
	validator "github.com/FJSDS/go-proto-validators"
)
...
func (p *plugin) generateProto2Message(file *generator.FileDescriptor, message *generator.Descriptor) {
	...
	for _, field := range message.Field {
		...
		if field.IsString() {
			p.generateStringValidator(variableName, ccTypeName, field.GetName(), fieldValidator)
		} else if p.isSupportedInt(field) {
			p.generateIntValidator(variableName, ccTypeName, field.GetName(), fieldValidator)
		} else if field.IsEnum() {
			p.generateEnumValidator(field, variableName, ccTypeName, field.GetName(), fieldValidator)
		} else if p.isSupportedFloat(field) {
			p.generateFloatValidator(variableName, ccTypeName, field.GetName(), fieldValidator)
		} else if field.IsBytes() {
			p.generateLengthValidator(variableName, ccTypeName, field.GetName(), fieldValidator)
		} else if field.IsMessage() {
			...
			p.In()
			p.P(`return `, p.validatorPkg.Use(), `.FieldError("`, field.GetName(), `", err)`)
			p.Out()
			p.P(`}`)
		}
		if repeated {
...
func (p *plugin) generateProto3Message(file *generator.FileDescriptor, message *generator.Descriptor) {
	...
	for _, oneof := range message.OneofDecl {
		...
		if oneofValidator.GetRequired() {
			oneOfName := oneof.GetName()//generator.CamelCase(oneof.GetName())
		...
		if field.IsString() {
			...
			if p.validatorWithMessageExists(fieldValidator) {
				if nullable && !repeated {
					...
					p.In()
					p.P(`return `, p.validatorPkg.Use(), `.FieldError("`, field.GetName(), `",`, p.fmtPkg.Use(), `.Errorf("message must exist"))`)
					p.Out()
					p.P(`}`)
				} else if repeated {
